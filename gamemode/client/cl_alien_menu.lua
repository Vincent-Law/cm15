...
-- Alien role selection menu - FIXED to use centralized config
...
function CM15_Menus.OpenAlienRoleMenu()
    ...
    for _, roleId in ipairs(roleOrder) do
        ...
        local modelPath = CM15_GetAlienModel(roleId)
        if not modelPath then
            -- Fallback if function doesn't exist yet
            modelPath = CM15_XENO_CONFIG and CM15_XENO_CONFIG.Models and CM15_XENO_CONFIG.Models[roleId]
            if not modelPath then
                -- Ultimate fallback
                modelPath = "models/cpthazama/avp/xeno/" .. string.lower(roleId) .. ".mdl"
            end
        end
        ...
        -- Model button - Now using the correct model path
