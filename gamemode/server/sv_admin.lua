...
-- Server-side command to inspect shared animations
concommand.Add("cm15_inspect_shared_anims", function(ply, cmd, args)
    if not IsValid(ply) then return end
    
    local sharedModel = "models/cpthazama/avp/xeno/animations_shared.mdl"
    
    -- Create temporary entity to inspect the shared model
    local testEnt = ents.Create("prop_dynamic")
    testEnt:SetModel(sharedModel)
    testEnt:SetPos(ply:GetPos() + Vector(100, 0, 0))
    testEnt:Spawn()
    
    ply:ChatPrint("=== INSPECTING SHARED ANIMATIONS MODEL ===")
    ply:ChatPrint("Model: " .. sharedModel)
    
    local sequenceCount = testEnt:GetSequenceCount()
    ply:ChatPrint("Total sequences: " .. sequenceCount)
    
    -- Look for strafe-related animations
    local strafeKeywords = {
        "strafe", "side", "left", "right", "walk_left", "walk_right", 
        "sidestep", "lateral", "move_left", "move_right"
    }
    
    ply:ChatPrint("=== SEARCHING FOR STRAFE ANIMATIONS ===")
    
    local foundStrafeAnims = {}
    
    for i = 0, sequenceCount - 1 do
        local seqName = testEnt:GetSequenceName(i)
        if seqName and seqName ~= "" then
            local lowerName = string.lower(seqName)
            
            -- Check if this animation might be strafe-related
            for _, keyword in ipairs(strafeKeywords) do
                if string.find(lowerName, keyword) then
                    table.insert(foundStrafeAnims, {id = i, name = seqName})
                    ply:ChatPrint(i .. ": " .. seqName .. " (POTENTIAL STRAFE)")
                    break
                end
            end
        end
    ...
    if #foundStrafeAnims == 0 then
        ply:ChatPrint("No obvious strafe animations found. Listing all sequences...")
        
        -- List all sequences if no obvious strafe ones found
        for i = 0, math.min(sequenceCount - 1, 100) do
            local seqName = testEnt:GetSequenceName(i)
            if seqName and seqName ~= "" then
