...
hook.Add("Think", "CM15_XenoDirectAnimationControl", function()
    for _, ply in pairs(player.GetAll()) do
        ...
        if not ply:OnGround() then
            ...
            local forceEnd = ply:GetNWFloat("ForceSequenceEnd", 0)
            if forceSeq > 0 and CurTime() < forceEnd then
                continue -- Let the forced sequence continue
            end
            
            -- Special case: if we just finished a crawling no-movement jump (203), use falling pose
            local lastSeq = lastSetSequence[steamID]
            if lastSeq == 203 then
                targetSeq = 147  -- falling_pose
                seqName = "falling_pose_from_crawl_jump"
            else
                -- In air animations - only set if not already in an air animation
                if currentSeq ~= 221 and currentSeq ~= 147 and currentSeq ~= 224 then
                    ...
                    -- Already in air animation, don't change it
                    continue
                end
            end
